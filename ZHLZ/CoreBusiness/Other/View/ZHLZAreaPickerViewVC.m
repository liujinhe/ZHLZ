...
@interface ZHLZAreaPickerViewVC () <UIPickerViewDataSource, UIPickerViewDelegate>
{
...
@property (nonatomic, strong) NSArray<ZHLZAreaModel *> *array;

@end

@implementation ZHLZAreaPickerViewVC
...
- (void)loadData {
    @weakify(self);
    if ([_orgId isNotBlank]) {
        self.task = [[ZHLZOtherVM sharedInstance] getAreaWithOrgId:_orgId withBlock:^(NSArray<ZHLZAreaModel *> * _Nonnull array) {
            @strongify(self);
            self.array = array;
...
- (IBAction)determineAction {
    ...
    [self dismissViewControllerAnimated:NO completion:^{
        @strongify(self);
        ZHLZAreaModel *model = self.array[self->_currentIndex];
