//
//  ZHLZTransactedPersonPickerViewVC.m
//  ZHLZ
//
...
@interface ZHLZTransactedPersonPickerViewVC () <UIPickerViewDataSource, UIPickerViewDelegate>
{
...
@property (nonatomic, strong) NSArray<ZHLZTransactedPersonModel *> *array;

@end

@implementation ZHLZTransactedPersonPickerViewVC
...
- (void)loadData {
    @weakify(self);
        self.task = [[ZHLZOtherVM sharedInstance] getTransactedPersonWithBlock:^(NSArray<ZHLZTransactedPersonModel *> * _Nonnull array) {
            @strongify(self);
            self.array = array;
...
- (IBAction)determineAction {
    ...
    [self dismissViewControllerAnimated:NO completion:^{
        @strongify(self);
        ZHLZTransactedPersonModel *model = self.array[self->_currentIndex];
        if (self.selectPickerBlock && model) {
            self.selectPickerBlock(model.userId, [model.userId isNotBlank] ? model.fullname : @"请选择");
        }
    }];
}
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    return self.array[row].fullname?:@"";
}
